...
const SPACE: &str = "space-worktree-run";
...
async fn run_with_worktree_spec_materializes_on_host_and_reuses() {
    ...
    core.project_actions
        .upsert(
            ...
            ProjectActionDraft {
                name: "Setup".into(),
                command: "printf 'setup\\n' >> \"$ZERON_WORKTREE_PATH/setup-ran\"".into(),
    ...
    wait_for(
        || first.join("setup-ran").is_file(),
        "durable worktree setup Action",
    )
    .await;
    assert_eq!(
        std::fs::read_to_string(first.join("setup-ran")).unwrap(),
        "setup\n"
    );
    ...
    assert_eq!(
        std::fs::read_to_string(first.join("setup-ran")).unwrap(),
        "setup\n",
        "reusing the worktree must not run setup twice"
    );
