...
/// A 2×3 miniature of [`gradient_spinner`] for compact personal-cut surfaces
/// that intentionally keep the original sunrise row tints.
pub fn mini_gradient_spinner(
    ...
    let tints = GSPIN_ROW_TINTS.map(|tint| gpui::rgb(tint).into());
