...
impl Shell {
    ...
    pub(super) fn sidebar_visible_order(&self, cx: &Context<Self>) -> Vec<String> {
        ...
        let ordered = if self.settings.sidebar_organization != SidebarOrganization::ByDevice {
            chats.into_iter().map(|chat| chat.id).collect::<Vec<_>>()
        } else {
        ...
        let saved_pins = profile_key
            .as_deref()
            .map(|key| self.settings.sidebar_pins(key))
            .unwrap_or_default();
        let pinned_order = self
            .pinned_session_drag
            .as_ref()
            .filter(|drag| {
                drag.filter == filter && profile_key.as_deref() == Some(&drag.profile_key)
            })
            .map_or(saved_pins, |drag| drag.visible_ids.as_slice());
    ...
    pub(super) fn render_active_rows(
        ...
        let sorted_ids: Vec<String> = rows.iter().map(|row| row.chat.id.clone()).collect();
        let ordered_ids = project_pinned_first(&sorted_ids, pinned_order);
        ...
        let active: HashSet<&str> = sorted_ids.iter().map(String::as_str).collect();
        ...
        // Pins are a device-local flat section above the organization chosen
        // for regular sessions. This keeps drag indices stable even when the
        // remaining rows are grouped by device.
        let unpinned_rows = rows.split_off(pinned_count);
        ...
        if self.settings.sidebar_organization == SidebarOrganization::ByDevice {
            let pinned_group = (pinned_count > 0).then(|| groups.remove(0));
            ...
            if let Some(pinned_group) = pinned_group {
