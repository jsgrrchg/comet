...
Local profiles retain profile-keyed device preferences. Synced profiles use only `sidebarPins` rows; they no longer import device-local lists or old `preferences/sidebar-v1` whole-list preferences.

Personal cut's one-time importer has been retired after the upgrade. Already imported pins and their ordering keys remain ordinary new-format records. Existing `personalCutLegacyImported` and `personalCutLegacyPending` fields are left untouched as inert metadata, and legacy lists are retained but ignored. No migration RPC or compatibility writer remains. A profile that missed the upgrade must pin its old items manually.
